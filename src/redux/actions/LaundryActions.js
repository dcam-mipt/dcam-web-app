    ...
    selectSlot: (data) => {
        return (dispatch, getState) => {
            if (
                ...
                !(getState().machines.machines.filter(i => i.objectId === data.machineId)[0].isDisabled && +moment(getState().machines.machines.filter(i => i.objectId === data.machineId)[0].chill_untill) >= +moment(data.timestamp)) &&
