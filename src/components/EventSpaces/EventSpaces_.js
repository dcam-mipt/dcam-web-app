        ...
        <Flex extra={`position: relative; @media (min-width: 320px) and (max-width: 480px) { height: 100%; justify-content: flex-start; }; @supports (-webkit-overflow-scrolling: touch) { height: 75vh; };`} >
            ...
            <WorkspaceWrapper>
                <LeftWrapper visible={!mobile_workspace_visible && !selector_visible}  >
                    ...
                    <Flex>
                        <LeftListItemTitle onClick={() => { set_dormitory_visible(!dormitory_visible) }} >
                        ...
                        <RequestsList>
                            {
                                dormitories && dormitories.map((item, index) => {
                                    return (
                                        <Flex row key={index} onClick={() => { set_selected_dormitory(item.objectId); set_selected_target(targets.filter(i => i.dormitory_id === item.objectId).length > 0 ? targets.filter(i => i.dormitory_id === item.objectId)[0].objectId : 0) }} >
                                            <ListIndicator visible={dormitory_visible} selected={selected_dormitory === item.objectId} />
                                            <ListItemText visible={dormitory_visible} >Общежитие №{item.number}</ListItemText>
                                        </Flex>
                                    )
                                })
                            }
