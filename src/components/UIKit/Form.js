        ...
        <Flex extra={`align-items: flex-start; justify-content; flex-start;`} >
            {
                array.map((question, question_index) => {
                    switch (question.type) {
                        ...
                        case `button`: return <Button key={question_index} disabled={!array[1].value || !array[3].value} backgroundColor={question.color} onClick={async () => { await question.action(); props.onCreate() }} >{question.text}</Button>
